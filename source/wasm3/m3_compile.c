...
//-------------------------------------------------------------------------------------------------------------------------
// Reference types (MVP extension).
// funcref/externref values are represented as i64-sized stack slots holding an IM3Function pointer
// (0 = null). This piggybacks on wasm3's existing i64 type so we don't have to extend the type system;
// pointers fit in 8 bytes on the supported platforms (aarch64 / x86_64). ref.is_null is a pure alias for
// i64.eqz at runtime, so it reuses op_i64_EqualToZero_r/s via d_unaryOpList in c_operations[].
...
M3Result  Compile_RefNull  (IM3Compilation o, m3opcode_t i_opcode)
{
    ...
    i8 heapType;
_   (ReadLEB_i7 (& heapType, & o->wasm, o->wasmEnd));           m3log (compile, d_indent " (ref.null heaptype = 0x%02x)", get_indention_string (o), (u8) heapType);
_   (PushConst (o, 0, c_m3Type_i64));
...
_   (ReadLEB_u32 (& functionIndex, & o->wasm, o->wasmEnd));
    ...
    _throwif (m3Err_functionLookupFailed, not function);
                                                                m3log (compile, d_indent " (ref.func [%d] '%s')", get_indention_string (o), functionIndex, m3_GetFunctionName (function));
_   (PushConst (o, (u64)(uintptr_t) function, c_m3Type_i64));
...
const M3OpInfo c_operations [] =
{
    ...
    // Reference Types (MVP extension). See Compile_RefNull / Compile_RefFunc above.
    // Designated indices skip the 0xc5..0xcf gap. ref.is_null piggybacks on i64.eqz.
    [0xd0] = M3OP( "ref.null",       1,  i_64,   d_emptyOpList,                      Compile_RefNull ),  // 0xd0
    [0xd1] = M3OP( "ref.is_null",    0,  i_32,   d_unaryOpList (i64, EqualToZero),   NULL ),             // 0xd1
    [0xd2] = M3OP( "ref.func",       1,  i_64,   d_emptyOpList,                      Compile_RefFunc ),  // 0xd2
