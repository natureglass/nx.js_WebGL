...
var gl = _screen.getContext("webgl2", {
  antialias: false,
  alpha: false,
  depth: false,
  stencil: false,
...
var vs = compile(gl.VERTEX_SHADER, vertexSource, "vs");
var fs = compile(gl.FRAGMENT_SHADER, fragmentSource, "fs");
...
function moveDrag(x, y) {
  ...
  lastX = x;
  lastY = y;
  targetYaw = clamp(targetYaw + dx * 32e-4, -0.78, 0.78);
  targetPitch = clamp(targetPitch - dy * 24e-4, -0.22, 0.18);
}
function endDrag() {
  dragActive = false;
  activeTouchId = null;
...
_screen.addEventListener("touchmove", (e) => {
  if (activeTouchId === null) return;
...
function waveHeightCPU(x, z, t) {
  let h = 0;
  h += 0.38 * Math.sin((x * WAVE_DIRS[0][0] + z * WAVE_DIRS[0][1]) * 0.09 + t * 0.82);
  h += 0.22 * Math.sin((x * WAVE_DIRS[1][0] + z * WAVE_DIRS[1][1]) * 0.155 + t * 1.18);
  h += 0.13 * Math.sin((x * WAVE_DIRS[2][0] + z * WAVE_DIRS[2][1]) * 0.29 + t * 1.75);
  h += 0.08 * Math.sin((x * WAVE_DIRS[3][0] + z * WAVE_DIRS[3][1]) * 0.52 + t * 2.15);
  h += (fbmCPU(x * 0.075 + t * 0.035, z * 0.075 - t * 0.018) - 0.5) * 0.2;
  return h;
}
var cubeCenterArr = new Float32Array(3);
var cubeRArr = new Float32Array(9);
var cubeInvRArr = new Float32Array(9);
var tmpA = new Float32Array(9);
var rY = new Float32Array(9);
var rX = new Float32Array(9);
var rZ = new Float32Array(9);
...
function updateCubePose(t) {
  ...
  cubeCenterArr[1] = seaBob + CUBE_HALF_SIZE * 0.33 + Math.sin(t * 0.72) * 0.1;
  cubeCenterArr[2] = ANCHOR_Z;
  const waveRoll = waveHeightCPU(ANCHOR_X + 2, ANCHOR_Z, t) - waveHeightCPU(ANCHOR_X - 2, ANCHOR_Z, t);
  const wavePitch = waveHeightCPU(ANCHOR_X, ANCHOR_Z + 2, t) - waveHeightCPU(ANCHOR_X, ANCHOR_Z - 2, t);
  rotYCPU(rY, t * 0.22);
...
function frame(now) {
  ...
  const dt = Math.min(0.05, Math.max(1e-3, (now - lastNow) * 1e-3));
  lastNow = now;
  pollGamepad(dt);
  yaw += (targetYaw - yaw) * 0.085;
  pitch += (targetPitch - pitch) * 0.085;
  fov += (targetFov - fov) * 0.075;
  const tSec = (now - start) * 1e-3;
  updateCubePose(tSec);
